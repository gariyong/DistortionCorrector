...
def select_img_from_video(
    video, board_pattern, select_all=False, wait_msec=10, wnd_name="Camera Calibration"
):
...
if __name__ == "__main__":
    distorted_video = cv.VideoCapture("./chessboard.mp4")
    board_pattern = (10, 7)
    board_cellsize = 0.025

    img_select = select_img_from_video(distorted_video, board_pattern)
    ...
    out = cv.VideoWriter()
    while True:
        valid_distorted, img_distorted = distorted_video.read()
        if not valid_distorted:
            break

        img_corrected = cv.undistort(img_distorted, K, dist_coeff)
        cv.imshow("correct distortion", img_corrected)
        out.open(
            "./distortion correction.avi",
            cv.VideoWriter_fourcc(*"XVID"),
            distorted_video.get(cv.CAP_PROP_FPS),
            (img_corrected.shape[1], img_corrected.shape[0]),
        )
        key = cv.waitKey(10)
        if key == 27:
            break
